...
class _VendorMenuPageState extends State<VendorMenuPage> {
  ...
  void _toggleAvailable(int index) async {
    setState(() {
      dishes[index]['available'] = !(dishes[index]['available'] ?? true);
    });
    await _saveDishes();
  }

  void _copyDish(int index) async {
    final dish = Map<String, dynamic>.from(dishes[index]);
    dish['id'] = DateTime.now().millisecondsSinceEpoch.toString();
    dish['name'] = dish['name'] + ' (Copy)';
    setState(() {
      dishes.insert(index + 1, dish);
    });
    await _saveDishes();
  }

  void _moveDish(int oldIndex, int newIndex) async {
    if (newIndex < 0 || newIndex >= dishes.length) return;
    setState(() {
      final dish = dishes.removeAt(oldIndex);
      dishes.insert(newIndex, dish);
    });
    await _saveDishes();
  }
          ...
          return ListTile(
            ...
            trailing: Row(
              ...
              children: [
                IconButton(
                  icon: Icon(
                    dish['available'] == false ? Icons.visibility_off : Icons.visibility,
                    color: dish['available'] == false ? Colors.grey : Colors.green),
                  tooltip: dish['available'] == false ? '上架' : '下架',
                  onPressed: () => _toggleAvailable(i),
                ),
                IconButton(
                  icon: Icon(Icons.copy, color: Colors.blue),
                  tooltip: '复制',
                  onPressed: () => _copyDish(i),
                ),
                IconButton(
                  icon: Icon(Icons.arrow_upward, color: Colors.deepPurple),
                  tooltip: '上移',
                  onPressed: i > 0 ? () => _moveDish(i, i - 1) : null,
                ),
                IconButton(
                  icon: Icon(Icons.arrow_downward, color: Colors.deepPurple),
                  tooltip: '下移',
                  onPressed: i < dishes.length - 1 ? () => _moveDish(i, i + 1) : null,
                ),
                IconButton(
